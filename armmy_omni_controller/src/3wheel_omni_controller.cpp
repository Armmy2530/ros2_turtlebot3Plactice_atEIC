...
#define WHEELS 3
#define WHEEL_Radian 0.0175 // dia 35mm = 0.0175
#define WHEEL_Offset 0.048 // Center of Base to Wheel + (Wheel thickness / 2) | 0.03875 + 0.00925 = 0.048
...
private:
  void cmdVel_Callback(const geometry_msgs::msg::Twist::SharedPtr msg) const
  {
    ...
    double wheel_1 = (vy - (omega * wheel_base_)) / wheel_radius_;
    double wheel_2 = ((-vy * 0.5) + (-vx * std::sin(PI/3)) - (omega * wheel_base_)) / wheel_radius_;
    double wheel_3 = ((-vy * 0.5) + ( vx * std::sin(PI/3)) - (omega * wheel_base_)) / wheel_radius_;

    if(cmd_vel_debug){
      RCLCPP_INFO(
          this->get_logger(),
          "\nReceived cmd_vel:%.2f, %.2f, %.2f \nWheel Speed= %.2f"" %.2f %.2f",
          msg->linear.x, msg->linear.y, msg->angular.z, wheel_1, wheel_2, wheel_3);
    }

    // Publish wheel velocities
    std_msgs::msg::Float64MultiArray wheel_vel_msg;
    wheel_vel_msg.data = {wheel_1, wheel_2, wheel_3};
