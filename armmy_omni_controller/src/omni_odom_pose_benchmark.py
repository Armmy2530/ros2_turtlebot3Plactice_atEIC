...
class OdomGroundTruthPlotter(Node):
    def __init__(self):
        ...
        self.create_subscription(Pose, '/gazebo_ground_truth/odom', self.ground_truth_callback, 10)
        ...
        # For velocity calculation from ground truth pose
        self.prev_gt_x = None
        self.prev_gt_y = None
        self.prev_gt_heading = None
    ...
    def ground_truth_callback(self, msg):
        ...
        # Calculate velocities from pose differences
        if (self.prev_gt_x is not None and self.prev_gt_y is not None and 
            self.prev_gt_heading is not None and self.prev_gt_time is not None):
            
            dt = (current_time - self.prev_gt_time).nanoseconds / 1e9  # Convert to seconds
            
            if dt > 0:
                # Linear velocities
                dx = position.x - self.prev_gt_x
                dy = position.y - self.prev_gt_y
                self.gt_vx = dx / dt
                self.gt_vy = dy / dt
                
                # Angular velocity
                dyaw = yaw - self.prev_gt_heading
                # Handle angle wrapping
                if dyaw > math.pi:
                    dyaw -= 2 * math.pi
                elif dyaw < -math.pi:
                    dyaw += 2 * math.pi
                self.gt_ang_vel = dyaw / dt

        # Store current values for next iteration
        self.prev_gt_x = position.x
        self.prev_gt_y = position.y
        self.prev_gt_heading = yaw
        self.prev_gt_time = current_time
...
def main():
    ...
    def on_key(event):
        if event.key == 'c':
            ...
            node.prev_gt_x = None
            node.prev_gt_y = None
            node.prev_gt_heading = None
            node.prev_gt_time = None
            node.gt_vx = 0.00
            node.gt_vy = 0.00
            node.gt_ang_vel = 0.00
